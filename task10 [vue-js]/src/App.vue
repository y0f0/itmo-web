<template>
    <div id="app">
        ...
        <Footer :usersAmount="Object.keys(users).length" :postsAmount="Object.keys(posts).length" />
    </div>
</template>
    ...
    beforeCreate() {
      this.$root.$on("onRegister", (login, name) => {
        if (login.length < 3 || login.length > 16) {
          this.$root.$emit("onRegisterValidationError", "The length of the lonia should be from 3 to 16");
          return
        }
        if (!login.match('^[a-z]+$')) {
          this.$root.$emit("onRegisterValidationError", "The login must consist only of lowercase Latin letters");
          return
        }

        const users = Object.values(this.users).filter(u => u.login === login);
        if (users.length === null) {
          this.$root.$emit("onRegisterValidationError", "The login must be unique");
          return
        }

        if (name.length < 1 || name.length > 32) {
          this.$root.$emit("onRegisterValidationError", "The length of the name must be 3 to 32");
          return
        }

        const id = Math.max(...Object.keys(this.users)) + 1;
        this.userId = id
        this.$set(this.users, id, {id, login, name, admin: false});
