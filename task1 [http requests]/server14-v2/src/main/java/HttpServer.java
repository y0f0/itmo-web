...
class HttpServer {
    ...
    private void process(Request request, Response response) throws IOException {
        if (!"GET".equals(request.getMethod())) { // только GET???
            response.setStatusCode(405);
            return;
        }
        String decodeUri = URLDecoder.decode(request.getUri(), StandardCharsets.UTF_8.name()); //decode

        File file = new File(root, decodeUri);
        if (file.isDirectory()){
        ...
        String ETag = Integer.toString(Integer.toString(body.length).hashCode());
