    ...
    public void updateAdmin(User actionUser) {
        try (Connection connection = DATA_SOURCE.getConnection()) {
            try (PreparedStatement statement = connection.prepareStatement("UPDATE User SET admin=? WHERE id=?", Statement.RETURN_GENERATED_KEYS)) {
                statement.setBoolean(1, actionUser.isAdmin());
                statement.setLong(2, actionUser.getId());
                if (statement.executeUpdate() != 1) {
                    throw new RepositoryException("Can't update User to admin.");
                }
            }
        } catch (SQLException e) {
            throw new RepositoryException("Can't update User to admin.", e);
        }
    }
}
