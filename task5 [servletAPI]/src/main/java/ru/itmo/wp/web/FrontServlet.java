...
public class FrontServlet extends HttpServlet {
    ...
    private void process(Route route, HttpServletRequest request, HttpServletResponse response)
        ...
        Set<Class<?>> acceptableParamTypes = new HashSet<>();
        acceptableParamTypes.add(HttpServletRequest.class);
        acceptableParamTypes.add(Map.class);
        ...
        for (Class<?> clazz = pageClass; method == null && clazz != null; clazz = clazz.getSuperclass()) {
            ...
            for (Method currentMethod : declaredMethods) {
                // Пусть есть класс у которого два метода экшон: экшон() и экшон(инт). Должны брать экшон().
                // [interface javax.servlet.http.HttpServletRequest, interface java.util.Map]
                if (currentMethod.getName().equals(route.getAction())) {
                    boolean ok = true;
                    for (Class<?> param : currentMethod.getParameterTypes()) {
                        if (!acceptableParamTypes.contains(param)) {
                            ok = false;
                            break;
                        }
                    }
        ...
        try {
            ...
            for (int i = 0; i < parameterTypes.length; i++) {
                args[i] = mapOfArgs.get(parameterTypes[i]);
            }
        ...
        try {
            ...
            if (langParameter != null) {
                if (langParameter.equals("ru") || langParameter.equals("en")) {
                    session.setAttribute("lang", langParameter);
                }
            }
            template = newTemplate(pageClass.getSimpleName() + suffix + ".ftlh");
        } catch (NotFoundException e) {
    ...
    private Template newTemplate(String templateName)
    ...
    private static class Route {
        ...
        private static Route newNotFoundRoute() {
            return new Route(
                    NotFoundPage.class.getName(), DEFAULT_ACTION
            );
        }

        private static Route newIndexRoute() {
            return new Route(
                    IndexPage.class.getName(), DEFAULT_ACTION
            );
        }
